...
def get_linP_Mpc(
    ...
    assert all(
        z in zs_out for z in zs
    ), "Requested redshifts not available in camb_results"
...
def create_LH_sample(
    dict_params_ranges: dict, nsamples: int = 400000, filename: str = "LHS_params.npz"
):
...
def generate_training_spectra(
    input_LH_filename: str, output_filename: str = "linear.dat"
):
    logger.info(
        f"Opening {PROJ_ROOT / 'data' / 'cosmopower_models' / input_LH_filename}"
    )
    ...
    for ii in tqdm(range(len(LH_params[params_list[0]]))):
        ...
        camb_results = get_camb_results(cosmo, zs=[3])

        k_Mpc, _, linP_Mpc = get_linP_Mpc(pars=cosmo, zs=[3], camb_results=camb_results)

        params_lhs = np.array([LH_params[param][ii] for param in params_list])
...
def cosmopower_prepare_training(
    params: List = ["H0", "mnu", "omega_cdm", "omega_b", "As", "ns"],
    Pk_filename: str = "linear.dat",
):
    k_modes = np.loadtxt(PROJ_ROOT / "data" / "cosmopower_models" / "k_modes.txt")
    linear_spectra_and_params = np.loadtxt(
        PROJ_ROOT / "data" / "cosmopower_models" / Pk_filename
    )
    ...
    valid_rows = ~(
        np.isnan(linear_spectra_and_params).any(axis=1)
    ...
    linear_parameters_dict = {
        params[i]: linear_parameters[:, i] for i in range(n_params)
    }
    linear_log_spectra_dict = {"modes": k_modes, "features": linear_log_spectra}
    np.savez(
        PROJ_ROOT / "data" / "cosmopower_models" / "camb_linear_params.npz",
        **linear_parameters_dict,
    )
    np.savez(
        PROJ_ROOT / "data" / "cosmopower_models" / "camb_linear_logpower.npz",
        **linear_log_spectra_dict,
    )


def cosmopower_train_model(model_params: List, model_save_filename: str = "Pk_cp_NN"):

    training_parameters = np.load(
        PROJ_ROOT / "data" / "cosmopower_models" / "camb_linear_params.npz"
    )
    ...
    training_parameters = {
        model_params[i]: training_parameters[model_params[i]]
    ...
    with tf.device(device):
        cp_nn.train(
            ...
            filename_saved_model=(
                PROJ_ROOT / "data" / "cosmopower_models" / model_save_filename
            ).as_posix(),
            # cooling schedule

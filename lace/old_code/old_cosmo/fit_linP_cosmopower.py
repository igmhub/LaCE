...
class linPCosmologyCosmopower:
    ...
    def __post_init__(self):
        self.cp_emulator = cp.cosmopower_NN(
            restore=True,
            restore_filename=PROJ_ROOT.as_posix()
            + f"/data/cosmopower_models/{self.cosmopower_model}",
        )

    @staticmethod
    def fit_polynomial(
        xmin: float, xmax: float, x: np.ndarray, y: np.ndarray, deg: int = 2
    ):
    ...
    @staticmethod
    def measure_Hz(params: dict, zstar: float):
        ...
        # Compute H(z)
        Hz = H0 * np.sqrt((Omega_bc + Omega_nu) * (1 + zstar) ** 3 + Omega_lambda)
        return Hz

    @staticmethod
    def convert_to_hMpc(Pk_Mpc: np.ndarray, k_Mpc: np.ndarray, h: np.ndarray):
    ...
    @staticmethod
    def convert_to_kms(
        params: dict, k_Mpc: np.ndarray, Pk_Mpc: np.ndarray, z_star: float
    ):
    ...
    @staticmethod
    def get_star_params(linP_kms: np.poly1d, kp_kms: float):
    ...
    def fit_linP_cosmology(self, chains_df, param_mapping, chunk_size=100_000):
        linP_cosmology_results = []
        # Calculate number of chunks needed
        n_chunks = len(chains_df) // chunk_size + (
            1 if len(chains_df) % chunk_size != 0 else 0
        )

        # Split DataFrame into chunks
        chunks = [
            chains_df.iloc[i * chunk_size : (i + 1) * chunk_size]
        ...
        param_info = [
            f"{value}: ({param_descriptions.get(key, 'The emulator does not use this parameter.')})"
            for key, value in param_mapping.items()
        ]
        logger.info(
            f"Using parameter mapping: {param_mapping}\n    "
            + "\n    ".join(param_info)
        )
        ...
        for chunk in chunks:  # create a dict of cosmological parameters
            ...
            for param, mapping in param_mapping.items():
                if param == "h":
                    ...
                    param == "omch2"
                    and "Omega_m" in param_mapping
                    and "h" in param_mapping
                ):
                    h = chunk[param_mapping["h"]]
                    params["ombh2"] = (
                        (chunk[param_mapping["Omega_m"]] - chunk[mapping] / h**2) * h**2
                    ).tolist()
            ...
            k_kms, Pk_kms = linPCosmologyCosmopower.convert_to_kms(
                params, k_Mpc, Pk_Mpc, z_star=self.z_star
            )
            ...
            for ii in tqdm(range(len(chunk)), desc="Processing samples"):
                ...
                results = linPCosmologyCosmopower.get_star_params(
                    linP_kms=poly_linP, kp_kms=self.kp_kms
                )

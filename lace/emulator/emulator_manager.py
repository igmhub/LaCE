...
def set_emulator(emulator_label, archive=None, drop_sim=None):
    ...
    if emulator_label in ["CH24_nyxcen_gpr", "CH24_mpgcen_gpr"]:
        return GPEmulator_multi(emulator_label=emulator_label)
    else:
        raise ValueError(
            f"Emulator {emulator_label} not supported. Supported emulators are CH24_nyxcen_gpr and CH24_mpgcen_gpr"
        )

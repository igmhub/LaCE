...
def P1D_emulator(archive=None, emu_algorithm=None, archive_label = 'Gadget', emulator_label=None, zmax=4.5, kmax_Mpc=4, ndeg=5, train=True, emu_type='polyfit', model_path=None, save_path=None, nepochs_nn=1):
    ...
    if emulator_label==None:
        ...
        if (archive==None)|(emu_algorithm==None):
            raise ValueError('If an emulator label is not provided, the archive and emulator type are required')

        if (emu_algorithm=='NN')&(archive_label=='Gadget'):
            emuparams = ['Delta2_p', 'n_p', 'mF', 'sigT_Mpc', 'gamma', 'kF_Mpc']
            emulator = NNEmulator(archive,emuparams, zmax=zmax, kmax_Mpc=kmax_Mpc, ndeg=ndeg,save_path=save_path )
